...
void MainWindow::on_actionFrom_camera_triggered()
{
    CameraWindow cameraDialog(this);
    if (cameraDialog.exec() == QDialog::Accepted) {
        QImage capturedImage = cameraDialog.getCapturedImage();

        if (!capturedImage.isNull()) {
            QMessageBox::information(this, "Image Captured", "Image has been successfully captured.");

            // Example of displaying the captured image in a QLabel
            // ui->imageLabel->setPixmap(QPixmap::fromImage(capturedImage).scaled(ui->imageLabel->size(), Qt::KeepAspectRatio));
        } else {
            QMessageBox::warning(this, "Capture Failed", "No image was captured.");
        }
    }
}
